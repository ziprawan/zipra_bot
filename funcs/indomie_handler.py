...
def main(msg: Message, *another):
    if msg.from_user == None:
        return msg.reply("Anonymous detected +++", True)
    button = InlineKeyboardButton("Refresh", callback_data=f'/indomie {msg.from_user.id}')
    markup = InlineKeyboardMarkup([[button]])
    teks = ['a', 'b', 'c', 'd', 'e']
    link_photo = [
        'https://awsimages.detik.net.id/community/media/visual/2021/10/18/indomie-10-besar-2.jpeg?w=700&q=90', 
    ...
    #     image.close()
    msg.reply_photo(terpilih, True, reply_markup=markup, caption=tekss)
