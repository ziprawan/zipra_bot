async def ban(event: Message, lang: Language, user: str | int | list[str | int], reason: str):
    ...
    if isinstance(user, list):
        ...
        msg = (await lang.get('restrict_batch')).format_map(
            Default(action="Ban", success=str(success), failed=str(failed)))
        ...
        if result == True:
            ...
            entities = [InputMessageEntityMentionName(offset=offs[0], length=lens[0],
                                                      user_id=InputUser(user_id=entity.id,
